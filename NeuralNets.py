...
class FullyConnected(NN):
    ...
    def forward_sample(self,x_initial,sample_no): 
            self.z[:,0,sample_no] = self.parameters["weights_in"] @ x_initial + self.parameters["input_bias"].flatten()
            self.a[:,0,sample_no] = self.parameters["weights_in"] @ x_initial + self.parameters["input_bias"].flatten()
        ...
        def grad_sample(self,ypred_sample,yactual_sample,sample_no,xinput_sample):
            ...
            if self.HiddenDim > 1:
                for i in range(self.HiddenDim -2, 0, -1):
                    ...
                    self.grad_bias_sample[:,i] = delta.flatten()
                    delta_prev = delta
            # input layers
